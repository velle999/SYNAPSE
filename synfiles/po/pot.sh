#!/usr/bin/env bash
# pot.sh — synfiles's message template, from BOTH of its source languages.
#
...
#
# ⛔ THE RECORD PROTOCOL IS NOT IN HERE. `--rec` emits states the window compares
# against and paths, names and mime types that are data; none of them is marked,
...
msgcat --use-first --no-location -o "$out/synfiles.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/synfiles.pot")" \
