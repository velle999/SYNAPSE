#!/usr/bin/env bash
# pot.sh — syn-clean's message template, from BOTH of its source languages.
#
...
# columns — `id label what bytes files root logins` — and data/syn-clean.qml
# keys off those names. The `id` column is the identifier every other command
# takes back: `syn-clean clean browsercache` is that string, and the window
# sends it. None of it is marked, and tests/i18n_test.sh proves it by running
# every offline --rec command under a catalog that translates EVERYTHING and
# diffing the bytes.
#
# ⚠ AND EVERY CATEGORY CARRIES AN id AND A LABEL, which is the trap here. They
# sit in the same struct one field apart: `browsercache` is what the program
# takes and "Browser cache" is what a person reads. Only the second is marked.
...
msgcat --use-first --no-location -o "$out/syn-clean.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/syn-clean.pot")" \
