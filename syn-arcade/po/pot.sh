#!/usr/bin/env bash
# pot.sh — syn-arcade's message template, from BOTH of its source languages.
#
...
# row NAMING THE COLUMNS and values both windows match on — a hud position is
# `top-left`, a fit scaler is `integer`, a pad axis is `ABS_X`. None of them is
# marked, and tests/i18n_test.sh proves it by running every offline --rec
# command under a real foreign locale and diffing the bytes.
#
# ⚠ TWO QML ENTRY POINTS, not one: the desktop window and big screen mode are
# separate quickshell processes over the same catalog. Both are in po/POTFILES,
# and a file missing from that list is English forever with nothing to say so.
#
# SynapseOS Project
...
msgcat --use-first --no-location -o "$out/syn-arcade.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/syn-arcade.pot")" \
