...
declare -gA SYN_T=(
  ...
  ["  Adding the [cachyos] repository..."]="  Добавляется репозиторий [cachyos]..."
  ...
  added. Add it later with:  synpkg cachyos enable-repo"]="Не удалось получить главный ключ CachyOS с keyserver.ubuntu.com.
  Без него подписанную связку ключей установить нельзя, поэтому [cachyos]
  не добавлен. Добавьте его позже:  synpkg cachyos enable-repo"
...
"
  ["Install SynapseOS"]="Установить SynapseOS"
  ["SynapseOS packages"]="Пакеты SynapseOS"
  ["Everything the system is made of. What you cannot drop is what something else you kept depends on — those are turned back on and named before anything is installed."]="Всё, из чего состоит система. То, что нельзя снять, нужно чему-то другому, что вы оставили, — такие пункты снова включаются и называются по имени до того, как что-либо будет установлено."
  ["SYNAPSE UI — the Wayland desktop"]="SYNAPSE UI — рабочий стол на Wayland"
  ["synapd — the local AI daemon"]="synapd — локальная служба ИИ"
  ["synsh — the AI-native shell"]="synsh — оболочка, встроенная в ИИ"
  ["synguard + kernel module"]="synguard + модуль ядра"
  ["synnet — network policy"]="synnet — сетевые правила"
  ["Software — the package manager"]="Software — менеджер пакетов"
  ["Files — the file manager"]="Файлы — файловый менеджер"
  ["Terminal (synui depends on it)"]="Терминал (synui без него не работает)"
  ["Settings"]="Настройки"
  ["Disks"]="Диски"
  ["Editor"]="Редактор"
  ["Calendar"]="Календарь"
  ["File Vault — a locked folder"]="Хранилище — запертая папка"
  ["Disk Cleanup — caches, and secure delete"]="Очистка диска — кэши и безопасное удаление"
  ["Malware Scan — a weekly ClamAV sweep"]="Проверка на вредоносное ПО — еженедельно, с ClamAV"
  ["syn-update — how fixes arrive"]="syn-update — так приходят исправления"
  ["syn — the top-level CLI"]="syn — главная командная строка"
  ["syn-model — fetch AI models"]="syn-model — загрузка моделей ИИ"
  ["syn-confine — the sandbox"]="syn-confine — песочница"
  ["fetch — the About OS readout"]="fetch — сводка о системе"
  ["Arcade — overlay, pads, big screen"]="Arcade — оверлей, геймпады, большой экран"
  ["cliamp — the music player"]="cliamp — музыкальный проигрыватель"
  ["Player — playlists, shuffle and history, on mpv"]="Player — плейлисты, случайный порядок и история, поверх mpv"
  ["Studio — photo darkroom and video"]="Studio — фотолаборатория и видео"
  ["GeForce NOW — cloud gaming in a browser"]="GeForce NOW — облачные игры в браузере"
  ["Remote desktop — reach this machine from another"]="Удалённый рабочий стол — доступ к этой машине с другой"
  ["Arsenal — BlackArch browser"]="Arsenal — обзор BlackArch"
  ["Speech engine — dictation and voice (~900 MB)"]="Речевой движок — диктовка и голос (~900 МБ)"
  ["Chibi — voice companion"]="Chibi — голосовой спутник"
  ["Vibe — AI coding assistant"]="Vibe — ИИ-помощник для кода"
  ["Animated wallpapers (~317 MB)"]="Живые обои (~317 МБ)"
  ["Nexus Chat (pulls in Firefox)"]="Nexus Chat (тянет за собой Firefox)"
  ["TEPRIS (pulls in Firefox)"]="TEPRIS (тянет за собой Firefox)"
  ["Web and communication"]="Веб и общение"
  ["None of this is ours; every name is in the Arch repositories. Firefox is on by default because an installed SynapseOS used to arrive with no browser at all."]="Ничего из этого не наше; каждое имя есть в репозиториях Arch. Firefox включён по умолчанию, потому что раньше установленный SynapseOS приходил вообще без браузера."
  ["Thunderbird — mail"]="Thunderbird — почта"
  ["KeePassXC — passwords"]="KeePassXC — пароли"
  ["Syncthing — file sync"]="Syncthing — синхронизация файлов"
  ["LocalSend — send to phone (Flatpak)"]="LocalSend — отправка на телефон (Flatpak)"
  ["Audio and video"]="Аудио и видео"
  ["Office and graphics"]="Офис и графика"
  ["Development and admin"]="Разработка и администрирование"
  ["VS Code (OSS build)"]="VS Code (сборка OSS)"
  ["7zip + unrar"]="7zip + unrar"
  ["Games, launchers and helpers"]="Игры, лаунчеры и вспомогательное"
  ["Steam is in the options below rather than here: it is the only one that turns on a second architecture and a third repository."]="Steam находится в параметрах ниже, а не здесь: только он включает вторую архитектуру и третий репозиторий."
  ...
  ["AntiMicroX — pad remap"]="AntiMicroX — переназначение геймпада"
  ["Welcome"]="Начало"
  ["Disk"]="Диск"
  ["Software"]="Программы"
  ...
  ["Summary"]="Сводка"
  ["Install"]="Установка"
  ["the installer's own media"]="носитель самого установщика"
  ["%1 GiB — SynapseOS needs at least %2 GiB"]="%1 ГиБ — SynapseOS нужно не меньше %2 ГиБ"
  ["No connection. SynapseOS downloads the base system while it installs, so this needs a working network before it can start."]="Нет соединения. SynapseOS загружает базовую систему прямо во время установки, поэтому до начала нужна работающая сеть."
  ["Choose a disk to install to."]="Выберите диск для установки."
  ["The encryption passphrase needs at least 8 characters."]="Парольная фраза шифрования должна быть не короче 8 символов."
  ["With neither the package manager nor the desktop, this install has no way to add either one back. Keep at least one."]="Без менеджера пакетов и без рабочего стола этой установке нечем вернуть ни то, ни другое. Оставьте хотя бы одно."
  ["A username is lower-case letters, digits, - and _, and cannot start with a digit."]="Имя пользователя состоит из строчных букв, цифр, - и _, и не может начинаться с цифры."
  ["Set a password for the account."]="Задайте пароль для учётной записи."
  ["The two passwords do not match."]="Пароли не совпадают."
  ["A locale is needed, e.g. en_US.UTF-8."]="Нужна локаль, например ru_RU.UTF-8."
  ["A timezone is needed, e.g. Europe/Lisbon."]="Нужен часовой пояс, например Europe/Moscow."
  ["printing"]="печать"
  ["%1 repo"]="репозиторий %1"
  ["Mode"]="Режим"
  ["Filesystem"]="Файловая система"
  ["%1 on LUKS2"]="%1 поверх LUKS2"
  ["%1 + snapshots"]="%1 + снимки"
  ["none"]="нет"
  ["Desktop"]="Рабочий стол"
  ["Locale"]="Локаль"
  ["%1   keys %2 / %3"]="%1   раскладки %2 / %3"
  ["Timezone"]="Часовой пояс"
  ["%1 package(s) — WITHOUT %2"]="%1 пакет(ов) — БЕЗ %2"
  ["%1 package(s)"]="%1 пакет(ов)"
  ["Options"]="Параметры"
  ["Could not write the install profile."]="Не удалось записать профиль установки."
  ["Installation complete."]="Установка завершена."
  ["Installation failed — see the log."]="Установка не удалась — смотрите журнал."
  ["No network connection"]="Нет сетевого соединения"
  ["The base system is downloaded while it installs, so this cannot start offline. Plug in a cable or join a network, then press Re-check — the answers on these pages are kept."]="Базовая система загружается во время установки, поэтому без сети начать нельзя. Подключите кабель или войдите в сеть, затем нажмите «Проверить снова» — ответы на этих страницах сохранятся."
  ["Checking…"]="Проверка…"
  ["Re-check"]="Проверить снова"
  ["Wi-Fi settings"]="Настройки Wi-Fi"
  ["This asks for a disk, an account and a few preferences, then hands the answers to the same installer the text version runs. Nothing is written to any disk until the last page, and that page says exactly what it is about to do."]="Здесь спрашивают диск, учётную запись и несколько настроек, а затем передают ответы тому же установщику, который запускает текстовая версия. До последней страницы ни на один диск ничего не пишется, и та страница точно говорит, что собирается сделать."
  ["A disk is partitioned and formatted"]="Диск размечается и форматируется"
  ["The base system and the SynapseOS packages are installed"]="Устанавливаются базовая система и пакеты SynapseOS"
  ["An account and a desktop are set up"]="Настраиваются учётная запись и рабочий стол"
  ["A bootloader is written"]="Записывается загрузчик"
  ["Partitioning an existing layout by hand is the text installer's ADVANCED mode — quit this and run \`syn-install\` in a terminal for that."]="Ручная разметка существующей схемы — это режим ADVANCED текстового установщика; для этого закройте окно и запустите \`syn-install\` в терминале."
  ["Where should SynapseOS go?"]="Куда поставить SynapseOS?"
  ["The installer's own media is listed and cannot be chosen."]="Носитель самого установщика показан в списке и выбрать его нельзя."
  ["No disks found."]="Диски не найдены."
  ["Erase the disk"]="Стереть диск"
  ["every partition and all data"]="все разделы и все данные"
  ["Install alongside"]="Установить рядом"
  ["use free space, UEFI only"]="использовать свободное место, только UEFI"
  ["Snapshots"]="Снимки"
  ["btrfs + limine only"]="только btrfs + limine"
  ["Encrypt the disk"]="Зашифровать диск"
  ["Passphrase"]="Парольная фраза"
  ["8 characters or more"]="8 символов или больше"
  ["What should be installed?"]="Что установить?"
  ["The SynapseOS core — the compositor, the daemons and the applications it is built on — is installed by every choice here."]="Ядро SynapseOS — композитор, службы и приложения, на которых он построен, — устанавливается при любом выборе здесь."
  ["Full"]="Полная"
  ["Standard + Steam + Nix + more software"]="Стандартная + Steam + Nix + больше программ"
  ["Standard"]="Стандартная"
  ["the SynapseOS suite, Firefox, AI model, Bluetooth, printing, Wine, phone"]="набор SynapseOS, Firefox, модель ИИ, Bluetooth, печать, Wine, телефон"
  ["Minimal"]="Минимальная"
  ["core daemons only — no apps, no software, no model"]="только основные службы — без приложений, без программ, без модели"
  ["Custom"]="Своя"
  ["tick every package yourself, ours and the ordinary software"]="отметить каждый пакет самому, наш и обычные программы"
  ["(required)"]="(обязательно)"
  ["Not packages: a repository, an architecture or a service. Each is a decision with a consequence that does not fit on a checkbox above."]="Это не пакеты: репозиторий, архитектура или служба. Каждый пункт — решение с последствиями, которые не помещаются в галочку выше."
  ["Printing (CUPS)"]="Печать (CUPS)"
  ["Wine — run Windows .exe/.msi"]="Wine — запуск .exe/.msi из Windows"
  ["KDE Connect — pair a phone"]="KDE Connect — подключить телефон"
  ["Steam + game stack + Proton (~3.1 GB)"]="Steam + игровой набор + Proton (~3,1 ГБ)"
  ["BlackArch repo — ~5000 tools, none installed"]="Репозиторий BlackArch — ~5000 инструментов, ни один не установлен"
  ["Nix + Home Manager"]="Nix + Home Manager"
  ["syn-update is off: this machine will have no way to receive another SynapseOS package. Fixing that later means installing it by hand from the ISO, or reinstalling."]="syn-update выключен: этой машине неоткуда будет получить следующий пакет SynapseOS. Исправить это потом означает поставить его вручную с ISO или переустановить систему."
  ["synui is off: this will not be a SynapseOS desktop. The Desktop page offers KDE, GNOME or no GUI."]="synui выключен: это не будет рабочим столом SynapseOS. На странице «Рабочий стол» есть KDE, GNOME или вообще без графики."
  ["AI model — downloaded during the install"]="Модель ИИ — загружается во время установки"
  ["~4.1 GB — recommended"]="~4,1 ГБ — рекомендуется"
  ["~2.2 GB — weaker"]="~2,2 ГБ — слабее"
  ["~0.4 GB — much weaker"]="~0,4 ГБ — намного слабее"
  ["None"]="Нет"
  ["AI stays inert"]="ИИ остаётся бездействующим"
  ["NVIDIA GPU inference"]="Вычисления на GPU NVIDIA"
  ["the CUDA runtime, ~4.7 GiB"]="среда выполнения CUDA, ~4,7 ГиБ"
  ["Who is this machine for?"]="Для кого эта машина?"
  ["Username"]="Имя пользователя"
  ["lower-case, no spaces"]="строчными, без пробелов"
  ["Full name (optional)"]="Полное имя (необязательно)"
  ["Password"]="Пароль"
  ["Password again"]="Пароль ещё раз"
  ["They do not match"]="Не совпадают"
  ["the native compositor"]="собственный композитор"
  ["synui is not selected"]="synui не выбран"
  ["headless"]="без графики"
  ["Language, keyboard and time"]="Язык, клавиатура и время"
  ["Pick a language and the other three follow it. The console keymap and the desktop layout are separate on purpose — Swedish is 'sv-latin1' to the console and 'se' to the desktop — so they can be changed on their own afterwards."]="Выберите язык, остальные три последуют за ним. Раскладка консоли и раскладка рабочего стола разделены намеренно — шведский для консоли это 'sv-latin1', а для рабочего стола 'se' — чтобы потом их можно было менять по отдельности."
  ["Language"]="Язык"
  ["sets the keyboard and the fonts too"]="задаёт также клавиатуру и шрифты"
  ["typed by hand — fonts cover as much as possible"]="введено вручную — шрифты покрывают столько, сколько могут"
  ["Sets the locale, both keyboard names and the font pack. Any locale glibc has can be typed instead."]="Задаёт локаль, оба имени раскладок и набор шрифтов. Вместо этого можно ввести любую локаль, известную glibc."
  ["The common zones first, then every name tzdata ships."]="Сначала распространённые зоны, затем каждое имя, которое приносит tzdata."
  ["Console keymap"]="Раскладка консоли"
  ["loadkeys — the text console and the greeter"]="loadkeys — текстовая консоль и экран входа"
  ["Every keymap this image can load. This one names a file loadkeys has to find, which is why it is not the same list as the desktop layout."]="Все раскладки, которые может загрузить этот образ. Здесь называется файл, который должен найти loadkeys, — поэтому список не тот же, что у рабочего стола."
  ["Desktop layout"]="Раскладка рабочего стола"
  ["XKB — the compositor"]="XKB — композитор"
  ["Desktop keyboard layout"]="Раскладка клавиатуры рабочего стола"
  ["The layouts xkbcommon can compile. 'uk' is a console keymap and not a layout here — the layout is 'gb'."]="Раскладки, которые может собрать xkbcommon. 'uk' — это раскладка консоли, а здесь не раскладка: здесь она называется 'gb'."
  ["Read this back"]="Перечитать всё"
  ["Nothing has been written yet. The next button is the one that starts."]="Пока ничего не записано. Следующая кнопка — та, что начинает."
  ["EVERY PARTITION ON %1 WILL BE DELETED"]="ВСЕ РАЗДЕЛЫ НА %1 БУДУТ УДАЛЕНЫ"
  ["SynapseOS will be installed into the free space on %1"]="SynapseOS будет установлен в свободное место на %1"
  ["SynapseOS is installed"]="SynapseOS установлен"
  ["The install stopped"]="Установка остановлена"
  ["Installing SynapseOS"]="Установка SynapseOS"
  ["Reboot and remove the installation media."]="Перезагрузите и извлеките установочный носитель."
  ["The log below is the whole story — the last lines say why."]="Журнал ниже рассказывает всё — последние строки говорят почему."
  ["This takes a while: the base system and the packages are downloaded, and an AI model is gigabytes on its own."]="Это займёт время: загружаются базовая система и пакеты, а модель ИИ сама по себе весит гигабайты."
  ["Back"]="Назад"
  ["Next"]="Далее"
  ["Reboot"]="Перезагрузить"
  ["Close"]="Закрыть"
  ["type to filter, or type a name that is not listed"]="печатайте для фильтра или введите имя, которого нет в списке"
  ["Nothing to list on this image — type the name instead."]="На этом образе нечего перечислять — введите имя вручную."
  ["Nothing matches — the row below uses what you typed."]="Совпадений нет — строка ниже возьмёт то, что вы ввели."
  ["Use “%1” as typed"]="Использовать «%1» как введено"
)
