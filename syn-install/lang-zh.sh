...
"
  ["Install SynapseOS"]="安装 SynapseOS"
  ["SynapseOS packages"]="SynapseOS 软件包"
  ["Everything the system is made of. What you cannot drop is what something else you kept depends on — those are turned back on and named before anything is installed."]="系统的全部组成。无法取消的项目，是你保留的其他内容所依赖的；在开始安装之前，它们会被重新启用并逐一列出。"
  ["SYNAPSE UI — the Wayland desktop"]="SYNAPSE UI — Wayland 桌面"
  ["synapd — the local AI daemon"]="synapd — 本地 AI 守护进程"
  ["synsh — the AI-native shell"]="synsh — AI 原生 shell"
  ["synguard + kernel module"]="synguard + 内核模块"
  ["synnet — network policy"]="synnet — 网络策略"
  ["Software — the package manager"]="Software — 软件包管理器"
  ["Files — the file manager"]="文件 — 文件管理器"
  ["Terminal (synui depends on it)"]="终端（synui 依赖它）"
  ["Settings"]="设置"
  ["Disks"]="磁盘"
  ["Editor"]="编辑器"
  ["Calendar"]="日历"
  ...
  ["syn-confine — the sandbox"]="syn-confine — 沙箱"
  ["fetch — the About OS readout"]="fetch — 系统概览"
  ["Arcade — overlay, pads, big screen"]="Arcade — 覆盖层、手柄、大屏"
  ["cliamp — the music player"]="cliamp — 音乐播放器"
  ["Player — playlists, shuffle and history, on mpv"]="Player — 播放列表、随机与历史，基于 mpv"
  ...
  ["Vibe — AI coding assistant"]="Vibe — AI 编程助手"
  ["Animated wallpapers (~317 MB)"]="动态壁纸（~317 MB）"
  ["Nexus Chat (pulls in Firefox)"]="Nexus Chat（会带上 Firefox）"
  ["TEPRIS (pulls in Firefox)"]="TEPRIS（会带上 Firefox）"
  ["Web and communication"]="网络与通讯"
  ["None of this is ours; every name is in the Arch repositories. Firefox is on by default because an installed SynapseOS used to arrive with no browser at all."]="这些都不是我们的；每个名字都在 Arch 仓库里。Firefox 默认开启，因为过去装好的 SynapseOS 里一个浏览器都没有。"
  ["Thunderbird — mail"]="Thunderbird — 邮件"
  ["KeePassXC — passwords"]="KeePassXC — 密码"
  ["Syncthing — file sync"]="Syncthing — 文件同步"
  ["LocalSend — send to phone (Flatpak)"]="LocalSend — 发送到手机（Flatpak）"
  ["Audio and video"]="音频与视频"
  ["Office and graphics"]="办公与图形"
  ["Development and admin"]="开发与管理"
  ["VS Code (OSS build)"]="VS Code（OSS 构建）"
  ["7zip + unrar"]="7zip + unrar"
  ["Games, launchers and helpers"]="游戏、启动器与辅助工具"
  ["Steam is in the options below rather than here: it is the only one that turns on a second architecture and a third repository."]="Steam 在下面的选项里而不在这里：它是唯一会启用第二种架构和第三个仓库的项目。"
  ...
  ["AntiMicroX — pad remap"]="AntiMicroX — 手柄重映射"
  ["Welcome"]="欢迎"
  ["Disk"]="磁盘"
  ["Software"]="软件"
  ["Account"]="账户"
  ...
  ["Summary"]="概要"
  ["Install"]="安装"
  ["the installer's own media"]="安装程序自身的介质"
  ["%1 GiB — SynapseOS needs at least %2 GiB"]="%1 GiB — SynapseOS 至少需要 %2 GiB"
  ["No connection. SynapseOS downloads the base system while it installs, so this needs a working network before it can start."]="没有连接。SynapseOS 在安装过程中下载基础系统，因此开始之前需要可用的网络。"
  ["Choose a disk to install to."]="选择要安装到的磁盘。"
  ["The encryption passphrase needs at least 8 characters."]="加密口令至少需要 8 个字符。"
  ["With neither the package manager nor the desktop, this install has no way to add either one back. Keep at least one."]="既没有软件包管理器也没有桌面，这次安装就没有办法把两者中的任何一个装回来。请至少保留一个。"
  ["A username is lower-case letters, digits, - and _, and cannot start with a digit."]="用户名由小写字母、数字、- 和 _ 组成，且不能以数字开头。"
  ["Set a password for the account."]="为该账户设置密码。"
  ["The two passwords do not match."]="两次输入的密码不一致。"
  ["A locale is needed, e.g. en_US.UTF-8."]="需要一个区域设置，例如 zh_CN.UTF-8。"
  ["A timezone is needed, e.g. Europe/Lisbon."]="需要一个时区，例如 Asia/Shanghai。"
  ["printing"]="打印"
  ["%1 repo"]="%1 仓库"
  ["Mode"]="模式"
  ["Filesystem"]="文件系统"
  ["%1 on LUKS2"]="LUKS2 上的 %1"
  ["%1 + snapshots"]="%1 + 快照"
  ["none"]="无"
  ["Desktop"]="桌面"
  ["Locale"]="区域设置"
  ["%1   keys %2 / %3"]="%1   键盘 %2 / %3"
  ["Timezone"]="时区"
  ["%1 package(s) — WITHOUT %2"]="%1 个软件包 — 不含 %2"
  ["%1 package(s)"]="%1 个软件包"
  ["Options"]="选项"
  ["Could not write the install profile."]="无法写入安装配置。"
  ["Installation complete."]="安装完成。"
  ["Installation failed — see the log."]="安装失败 — 请查看日志。"
  ["No network connection"]="没有网络连接"
  ["The base system is downloaded while it installs, so this cannot start offline. Plug in a cable or join a network, then press Re-check — the answers on these pages are kept."]="基础系统在安装过程中下载，因此离线无法开始。请插上网线或加入网络，然后按“重新检测”——这些页面上的答案会保留。"
  ["Checking…"]="正在检测…"
  ["Re-check"]="重新检测"
  ["Wi-Fi settings"]="Wi-Fi 设置"
  ["This asks for a disk, an account and a few preferences, then hands the answers to the same installer the text version runs. Nothing is written to any disk until the last page, and that page says exactly what it is about to do."]="这里会询问磁盘、账户和几项偏好，然后把答案交给文本版所运行的同一个安装程序。直到最后一页之前不会向任何磁盘写入，而那一页会确切说明它将要做什么。"
  ["A disk is partitioned and formatted"]="对磁盘分区并格式化"
  ["The base system and the SynapseOS packages are installed"]="安装基础系统和 SynapseOS 软件包"
  ["An account and a desktop are set up"]="建立账户和桌面"
  ["A bootloader is written"]="写入引导程序"
  ["Partitioning an existing layout by hand is the text installer's ADVANCED mode — quit this and run \`syn-install\` in a terminal for that."]="手工为现有布局分区属于文本安装程序的 ADVANCED 模式 — 请关闭本窗口，在终端里运行 \`syn-install\`。"
  ["Where should SynapseOS go?"]="SynapseOS 装到哪里？"
  ["The installer's own media is listed and cannot be chosen."]="安装程序自身的介质会列出，但不能选择。"
  ["No disks found."]="未找到磁盘。"
  ["Erase the disk"]="擦除磁盘"
  ["every partition and all data"]="每一个分区和全部数据"
  ["Install alongside"]="并存安装"
  ["use free space, UEFI only"]="使用空闲空间，仅限 UEFI"
  ["Snapshots"]="快照"
  ["btrfs + limine only"]="仅 btrfs + limine"
  ["Encrypt the disk"]="加密磁盘"
  ["Passphrase"]="口令"
  ["8 characters or more"]="8 个字符或更多"
  ["What should be installed?"]="要安装什么？"
  ["The SynapseOS core — the compositor, the daemons and the applications it is built on — is installed by every choice here."]="SynapseOS 的核心 — 合成器、各守护进程以及它所依赖的应用 — 在这里无论选哪一项都会安装。"
  ["Full"]="完整"
  ["Standard + Steam + Nix + more software"]="标准 + Steam + Nix + 更多软件"
  ["Standard"]="标准"
  ["the SynapseOS suite, Firefox, AI model, Bluetooth, printing, Wine, phone"]="SynapseOS 套件、Firefox、AI 模型、蓝牙、打印、Wine、手机"
  ["Minimal"]="最小"
  ["core daemons only — no apps, no software, no model"]="仅核心守护进程 — 没有应用，没有软件，没有模型"
  ["Custom"]="自定义"
  ["tick every package yourself, ours and the ordinary software"]="每个软件包都自己勾选，我们的和普通软件都在内"
  ["(required)"]="（必需）"
  ["Not packages: a repository, an architecture or a service. Each is a decision with a consequence that does not fit on a checkbox above."]="这些不是软件包：是仓库、架构或服务。每一项都是一个决定，其后果放不进上面的复选框。"
  ["Printing (CUPS)"]="打印（CUPS）"
  ["Wine — run Windows .exe/.msi"]="Wine — 运行 Windows 的 .exe/.msi"
  ["KDE Connect — pair a phone"]="KDE Connect — 配对手机"
  ["Steam + game stack + Proton (~3.1 GB)"]="Steam + 游戏组件 + Proton（~3.1 GB）"
  ["BlackArch repo — ~5000 tools, none installed"]="BlackArch 仓库 — 约 5000 个工具，一个也不安装"
  ["Nix + Home Manager"]="Nix + Home Manager"
  ["syn-update is off: this machine will have no way to receive another SynapseOS package. Fixing that later means installing it by hand from the ISO, or reinstalling."]="syn-update 已关闭：这台机器将没有办法再收到任何 SynapseOS 软件包。以后要补救，只能从 ISO 手工安装，或者重装。"
  ["synui is off: this will not be a SynapseOS desktop. The Desktop page offers KDE, GNOME or no GUI."]="synui 已关闭：这将不是一个 SynapseOS 桌面。桌面页面提供 KDE、GNOME 或不装图形界面。"
  ["AI model — downloaded during the install"]="AI 模型 — 安装过程中下载"
  ["~4.1 GB — recommended"]="~4.1 GB — 推荐"
  ["~2.2 GB — weaker"]="~2.2 GB — 能力较弱"
  ["~0.4 GB — much weaker"]="~0.4 GB — 能力弱得多"
  ["None"]="无"
  ["AI stays inert"]="AI 保持不工作"
  ["NVIDIA GPU inference"]="NVIDIA GPU 推理"
  ["the CUDA runtime, ~4.7 GiB"]="CUDA 运行时，约 4.7 GiB"
  ["Who is this machine for?"]="这台机器是给谁用的？"
  ["Username"]="用户名"
  ["lower-case, no spaces"]="小写，不含空格"
  ["Full name (optional)"]="全名（可选）"
  ["Password"]="密码"
  ["Password again"]="再次输入密码"
  ["They do not match"]="两者不一致"
  ["the native compositor"]="原生合成器"
  ["synui is not selected"]="未选择 synui"
  ["headless"]="无图形界面"
  ["Language, keyboard and time"]="语言、键盘与时间"
  ["Pick a language and the other three follow it. The console keymap and the desktop layout are separate on purpose — Swedish is 'sv-latin1' to the console and 'se' to the desktop — so they can be changed on their own afterwards."]="选好语言，其余三项会跟着它。控制台键盘映射和桌面布局是有意分开的 — 瑞典语在控制台上是 'sv-latin1'，在桌面上是 'se' — 这样之后可以各自单独修改。"
  ["Language"]="语言"
  ["sets the keyboard and the fonts too"]="同时设置键盘和字体"
  ["typed by hand — fonts cover as much as possible"]="手工输入 — 字体会尽可能覆盖"
  ["Sets the locale, both keyboard names and the font pack. Any locale glibc has can be typed instead."]="设置区域、两个键盘名称和字体包。也可以改为输入 glibc 认识的任何区域设置。"
  ["The common zones first, then every name tzdata ships."]="先是常用时区，然后是 tzdata 提供的每一个名称。"
  ["Console keymap"]="控制台键盘映射"
  ["loadkeys — the text console and the greeter"]="loadkeys — 文本控制台与登录界面"
  ["Every keymap this image can load. This one names a file loadkeys has to find, which is why it is not the same list as the desktop layout."]="此映像能加载的每个键盘映射。这里指的是 loadkeys 必须找到的文件名，所以它与桌面布局不是同一份列表。"
  ["Desktop layout"]="桌面布局"
  ["XKB — the compositor"]="XKB — 合成器"
  ["Desktop keyboard layout"]="桌面键盘布局"
  ["The layouts xkbcommon can compile. 'uk' is a console keymap and not a layout here — the layout is 'gb'."]="xkbcommon 能够编译的布局。'uk' 是控制台键盘映射，在这里不是布局 — 这里的布局是 'gb'。"
  ["Read this back"]="再看一遍"
  ["Nothing has been written yet. The next button is the one that starts."]="到目前为止还没有写入任何东西。下一个按钮就是开始的那一个。"
  ["EVERY PARTITION ON %1 WILL BE DELETED"]="%1 上的每一个分区都将被删除"
  ["SynapseOS will be installed into the free space on %1"]="SynapseOS 将安装到 %1 的空闲空间中"
  ["SynapseOS is installed"]="SynapseOS 已安装"
  ["The install stopped"]="安装已停止"
  ["Installing SynapseOS"]="正在安装 SynapseOS"
  ["Reboot and remove the installation media."]="重新启动，并取出安装介质。"
  ["The log below is the whole story — the last lines say why."]="下面的日志记录了全部经过 — 最后几行说明原因。"
  ["This takes a while: the base system and the packages are downloaded, and an AI model is gigabytes on its own."]="这需要一些时间：基础系统和软件包都要下载，而 AI 模型本身就有好几个 GB。"
  ["Back"]="返回"
  ["Next"]="下一步"
  ["Reboot"]="重新启动"
  ["Close"]="关闭"
  ["type to filter, or type a name that is not listed"]="输入以筛选，或输入一个未列出的名称"
  ["Nothing to list on this image — type the name instead."]="此映像上没有可列出的内容 — 请直接输入名称。"
  ["Nothing matches — the row below uses what you typed."]="没有匹配项 — 下面这一行会使用你输入的内容。"
  ["Use “%1” as typed"]="按输入使用“%1”"
)
