...
# The start menu is synui's own panel now (synui/src/menu.c), not a GtkBuilder
# menu behind this badge. waybar could never arrow-navigate one: it asks for
# keyboard_interactivity NONE and never revises it, so its menu is handed no
# keyboard focus. synui scans the installed .desktop files itself when the menu
# opens, so there is no generated XML to seed here and nothing to regenerate on
# boot — which also retires the XML/menu-actions pair that drifted and once
# mapped menu entries to the wrong commands. network-menu.xml above is now the
# only GtkBuilder menu left; the network module still uses waybar's menu feature.
