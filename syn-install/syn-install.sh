...
SEL_COMPONENTS=(
    ...
    "comp_synscan|1|1|core|syn-scan|Malware Scan|malware scanner"
...
# synui config
#
# ⛔ FROM /etc/skel, NOT A HEREDOC OF ITS OWN. synui ships the house synuirc as
# /etc/skel/.config/synui/synuirc (synui/config/skel/synuirc), and `useradd -m`
# above has already copied it into this home. It used to be written here, into
# the first account only — so every account made after it, in syn-settings'
# Users pane or by hand, logged into synui's bare compiled-in defaults: a
# terminal, no bar, no polkit agent. One file now, and every account starts
# from it, this one included.
#
# ⚠ CHECKED, NOT ASSUMED. A synui built before the file shipped would leave this
# home without one — and the xkb line below would then CREATE a synuirc holding
# only that, which replaces synui's defaults outright (a file naming any
# autostart replaces the built-in list). So a missing file is said, and the
# three lines a working desktop cannot go without are written in its place.
# Only where synui IS installed: it is a checkbox like everything else, and a
# machine without it has no desktop for this file to describe.
#
# ⚠ The directory is made either way — the xkb and KDE Connect lines below
# append into it, and appending into a directory that is not there fails.
mkdir -p "/mnt/home/$NEW_USER/.config/synui"
if [ ! -f "/mnt/home/$NEW_USER/.config/synui/synuirc" ] && [ -x /mnt/usr/bin/synui ]; then
    warn "synui did not provide /etc/skel/.config/synui/synuirc — writing a minimal one (no house theme)."
    printf '%s\n' 'terminal = syntty' 'autostart = synui-bar' \
        'autostart = /usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1' \
        > "/mnt/home/$NEW_USER/.config/synui/synuirc"
fi
...
# fonts. Appended to the copy /etc/skel gave this account, which is the same file
# every account starts from — the layout is this machine's, not the file's.
