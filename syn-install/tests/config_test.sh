...
#
# ⚠ THE FALLBACK BELOW IS THIS WINDOW'S LITERAL, NOT theme.json's ACCENT, and
# that is a separate bug left standing here on purpose. syn-install-gui's own
# themed() accepts a "#rrggbb" STRING or a {r,g,b} float object; synui writes
# accent/glyph/bar/popup as [r,g,b] ARRAYS, which it matches neither of, so the
# installer has never taken a colour from a theme at all. It is invisible on the
# ISO, which usually has no theme.json to read. What is checked here is the only
# thing this change claims: with the wallpaper accent refused, the window keeps
# whatever it had before, and with it offered, it takes it.

if [ -f "$gui" ] && command -v quickshell >/dev/null 2>&1; then
    WPT=$(mktemp -d)
...
WPJSON
    ...
    check "the measured wallpaper accent reaches the window" "#6479ff" \
          "$(wp_accent yes yes)"
    check "wallpaper_accent off leaves the theme's accent alone" "#33ccff" \
          "$(wp_accent no yes)"
    check "a wallpaper with no usable hue falls back to the theme" "#33ccff" \
          "$(wp_accent yes no)"
    rm -rf "$WPT"
else
