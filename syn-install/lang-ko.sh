...
declare -gA SYN_T=(
  ...
  ["  Adding the [cachyos] repository..."]="  [cachyos] 저장소를 추가하는 중..."
  ...
  added. Add it later with:  synpkg cachyos enable-repo"]="keyserver.ubuntu.com 에서 CachyOS 마스터 키를 받지 못했습니다.
  그것이 없으면 서명된 키링을 설치할 수 없어 [cachyos] 은
  추가하지 않습니다. 나중에 추가하려면:  synpkg cachyos enable-repo"
...
"
  ["Install SynapseOS"]="SynapseOS 설치"
  ["SynapseOS packages"]="SynapseOS 패키지"
  ["Everything the system is made of. What you cannot drop is what something else you kept depends on — those are turned back on and named before anything is installed."]="시스템을 이루는 모든 것. 해제할 수 없는 항목은 남겨 둔 다른 항목이 의존하는 것으로, 설치가 시작되기 전에 다시 켜지고 이름이 표시됩니다."
  ["SYNAPSE UI — the Wayland desktop"]="SYNAPSE UI — Wayland 데스크톱"
  ["synapd — the local AI daemon"]="synapd — 로컬 AI 데몬"
  ["synsh — the AI-native shell"]="synsh — AI 네이티브 셸"
  ["synguard + kernel module"]="synguard + 커널 모듈"
  ["synnet — network policy"]="synnet — 네트워크 정책"
  ["Software — the package manager"]="Software — 패키지 관리자"
  ["Files — the file manager"]="파일 — 파일 관리자"
  ["Terminal (synui depends on it)"]="터미널 (synui가 필요로 함)"
  ["Settings"]="설정"
  ["Disks"]="디스크"
  ["Editor"]="편집기"
  ["Calendar"]="달력"
  ...
  ["syn-confine — the sandbox"]="syn-confine — 샌드박스"
  ["fetch — the About OS readout"]="fetch — 시스템 요약"
  ["Arcade — overlay, pads, big screen"]="Arcade — 오버레이, 패드, 큰 화면"
  ["cliamp — the music player"]="cliamp — 음악 재생기"
  ["Player — playlists, shuffle and history, on mpv"]="Player — 재생 목록, 무작위와 기록, mpv 기반"
  ...
  ["Vibe — AI coding assistant"]="Vibe — AI 코딩 도우미"
  ["Animated wallpapers (~317 MB)"]="움직이는 배경화면 (~317 MB)"
  ["Nexus Chat (pulls in Firefox)"]="Nexus Chat (Firefox도 함께 설치됨)"
  ["TEPRIS (pulls in Firefox)"]="TEPRIS (Firefox도 함께 설치됨)"
  ["Web and communication"]="웹과 통신"
  ["None of this is ours; every name is in the Arch repositories. Firefox is on by default because an installed SynapseOS used to arrive with no browser at all."]="여기 있는 것은 우리 것이 아니며, 모든 이름은 Arch 저장소에 있습니다. Firefox가 기본으로 켜져 있는 이유는, 예전에는 설치된 SynapseOS에 브라우저가 하나도 없었기 때문입니다."
  ["Thunderbird — mail"]="Thunderbird — 메일"
  ["KeePassXC — passwords"]="KeePassXC — 비밀번호"
  ["Syncthing — file sync"]="Syncthing — 파일 동기화"
  ["LocalSend — send to phone (Flatpak)"]="LocalSend — 휴대폰으로 보내기 (Flatpak)"
  ["Audio and video"]="오디오와 비디오"
  ["Office and graphics"]="사무와 그래픽"
  ["Development and admin"]="개발과 관리"
  ["VS Code (OSS build)"]="VS Code (OSS 빌드)"
  ["7zip + unrar"]="7zip + unrar"
  ["Games, launchers and helpers"]="게임, 런처, 보조 도구"
  ["Steam is in the options below rather than here: it is the only one that turns on a second architecture and a third repository."]="Steam은 여기가 아니라 아래 옵션에 있습니다. 두 번째 아키텍처와 세 번째 저장소를 켜는 유일한 항목이기 때문입니다."
  ...
  ["AntiMicroX — pad remap"]="AntiMicroX — 패드 재배치"
  ["Welcome"]="환영"
  ["Disk"]="디스크"
  ["Software"]="소프트웨어"
  ["Account"]="계정"
  ...
  ["Summary"]="요약"
  ["Install"]="설치"
  ["the installer's own media"]="설치 프로그램 자신의 매체"
  ["%1 GiB — SynapseOS needs at least %2 GiB"]="%1 GiB — SynapseOS에는 최소 %2 GiB가 필요합니다"
  ["No connection. SynapseOS downloads the base system while it installs, so this needs a working network before it can start."]="연결이 없습니다. SynapseOS는 설치하는 동안 기본 시스템을 내려받으므로, 시작하기 전에 동작하는 네트워크가 필요합니다."
  ["Choose a disk to install to."]="설치할 디스크를 선택하세요."
  ["The encryption passphrase needs at least 8 characters."]="암호화 암호문은 최소 8자가 필요합니다."
  ["With neither the package manager nor the desktop, this install has no way to add either one back. Keep at least one."]="패키지 관리자도 데스크톱도 없으면, 이 설치본은 둘 중 어느 것도 다시 추가할 방법이 없습니다. 최소한 하나는 남겨 두세요."
  ["A username is lower-case letters, digits, - and _, and cannot start with a digit."]="사용자 이름은 소문자, 숫자, - 와 _ 로 이루어지며 숫자로 시작할 수 없습니다."
  ["Set a password for the account."]="계정의 비밀번호를 정하세요."
  ["The two passwords do not match."]="두 비밀번호가 일치하지 않습니다."
  ["A locale is needed, e.g. en_US.UTF-8."]="로캘이 필요합니다. 예: ko_KR.UTF-8"
  ["A timezone is needed, e.g. Europe/Lisbon."]="시간대가 필요합니다. 예: Asia/Seoul"
  ["printing"]="인쇄"
  ["%1 repo"]="%1 저장소"
  ["Mode"]="방식"
  ["Filesystem"]="파일 시스템"
  ["%1 on LUKS2"]="LUKS2 위의 %1"
  ["%1 + snapshots"]="%1 + 스냅숏"
  ["none"]="없음"
  ["Desktop"]="데스크톱"
  ["Locale"]="로캘"
  ["%1   keys %2 / %3"]="%1   키 %2 / %3"
  ["Timezone"]="시간대"
  ["%1 package(s) — WITHOUT %2"]="패키지 %1개 — %2 제외"
  ["%1 package(s)"]="패키지 %1개"
  ["Options"]="옵션"
  ["Could not write the install profile."]="설치 프로파일을 기록하지 못했습니다."
  ["Installation complete."]="설치가 끝났습니다."
  ["Installation failed — see the log."]="설치에 실패했습니다 — 로그를 보세요."
  ["No network connection"]="네트워크 연결 없음"
  ["The base system is downloaded while it installs, so this cannot start offline. Plug in a cable or join a network, then press Re-check — the answers on these pages are kept."]="기본 시스템은 설치 중에 내려받으므로 오프라인으로는 시작할 수 없습니다. 케이블을 꽂거나 네트워크에 접속한 뒤 다시 확인을 누르세요 — 이 페이지들에 입력한 내용은 그대로 유지됩니다."
  ["Checking…"]="확인 중…"
  ["Re-check"]="다시 확인"
  ["Wi-Fi settings"]="Wi-Fi 설정"
  ["This asks for a disk, an account and a few preferences, then hands the answers to the same installer the text version runs. Nothing is written to any disk until the last page, and that page says exactly what it is about to do."]="여기서는 디스크와 계정, 몇 가지 설정을 묻고, 그 답을 텍스트 버전이 실행하는 것과 같은 설치 프로그램에 넘깁니다. 마지막 페이지까지는 어떤 디스크에도 아무것도 쓰지 않으며, 그 페이지가 무엇을 할지 정확히 알려 줍니다."
  ["A disk is partitioned and formatted"]="디스크를 분할하고 포맷합니다"
  ["The base system and the SynapseOS packages are installed"]="기본 시스템과 SynapseOS 패키지를 설치합니다"
  ["An account and a desktop are set up"]="계정과 데스크톱을 설정합니다"
  ["A bootloader is written"]="부트로더를 기록합니다"
  ["Partitioning an existing layout by hand is the text installer's ADVANCED mode — quit this and run \`syn-install\` in a terminal for that."]="기존 배치를 손으로 분할하는 것은 텍스트 설치 프로그램의 ADVANCED 모드입니다 — 그러려면 이 창을 닫고 터미널에서 \`syn-install\`을 실행하세요."
  ["Where should SynapseOS go?"]="SynapseOS를 어디에 설치할까요?"
  ["The installer's own media is listed and cannot be chosen."]="설치 프로그램 자신의 매체는 목록에 나오지만 선택할 수 없습니다."
  ["No disks found."]="디스크를 찾지 못했습니다."
  ["Erase the disk"]="디스크 지우기"
  ["every partition and all data"]="모든 파티션과 모든 데이터"
  ["Install alongside"]="나란히 설치"
  ["use free space, UEFI only"]="빈 공간 사용, UEFI 전용"
  ["Snapshots"]="스냅숏"
  ["btrfs + limine only"]="btrfs + limine 전용"
  ["Encrypt the disk"]="디스크 암호화"
  ["Passphrase"]="암호문"
  ["8 characters or more"]="8자 이상"
  ["What should be installed?"]="무엇을 설치할까요?"
  ["The SynapseOS core — the compositor, the daemons and the applications it is built on — is installed by every choice here."]="SynapseOS의 핵심 — 컴포지터, 데몬들, 그 위에 얹힌 응용 프로그램 — 은 여기서 무엇을 고르든 설치됩니다."
  ["Full"]="전체"
  ["Standard + Steam + Nix + more software"]="표준 + Steam + Nix + 더 많은 소프트웨어"
  ["Standard"]="표준"
  ["the SynapseOS suite, Firefox, AI model, Bluetooth, printing, Wine, phone"]="SynapseOS 모음, Firefox, AI 모델, 블루투스, 인쇄, Wine, 휴대폰"
  ["Minimal"]="최소"
  ["core daemons only — no apps, no software, no model"]="핵심 데몬만 — 앱 없음, 소프트웨어 없음, 모델 없음"
  ["Custom"]="사용자 지정"
  ["tick every package yourself, ours and the ordinary software"]="모든 패키지를 직접 선택 (우리 것과 일반 소프트웨어 모두)"
  ["(required)"]="(필수)"
  ["Not packages: a repository, an architecture or a service. Each is a decision with a consequence that does not fit on a checkbox above."]="패키지가 아닙니다. 저장소, 아키텍처, 서비스입니다. 각각은 위의 체크 상자에 담기지 않는 결과를 가진 결정입니다."
  ["Printing (CUPS)"]="인쇄 (CUPS)"
  ["Wine — run Windows .exe/.msi"]="Wine — Windows .exe/.msi 실행"
  ["KDE Connect — pair a phone"]="KDE Connect — 휴대폰 연결"
  ["Steam + game stack + Proton (~3.1 GB)"]="Steam + 게임 구성 + Proton (~3.1 GB)"
  ["BlackArch repo — ~5000 tools, none installed"]="BlackArch 저장소 — 도구 약 5000개, 설치되는 것은 없음"
  ["Nix + Home Manager"]="Nix + Home Manager"
  ["syn-update is off: this machine will have no way to receive another SynapseOS package. Fixing that later means installing it by hand from the ISO, or reinstalling."]="syn-update가 꺼져 있습니다. 이 컴퓨터는 앞으로 어떤 SynapseOS 패키지도 받을 방법이 없습니다. 나중에 고치려면 ISO에서 손으로 설치하거나 다시 설치해야 합니다."
  ["synui is off: this will not be a SynapseOS desktop. The Desktop page offers KDE, GNOME or no GUI."]="synui가 꺼져 있습니다. 이것은 SynapseOS 데스크톱이 아닙니다. 데스크톱 페이지에서 KDE, GNOME 또는 GUI 없음을 고를 수 있습니다."
  ["AI model — downloaded during the install"]="AI 모델 — 설치 중에 내려받음"
  ["~4.1 GB — recommended"]="~4.1 GB — 권장"
  ["~2.2 GB — weaker"]="~2.2 GB — 성능이 낮음"
  ["~0.4 GB — much weaker"]="~0.4 GB — 성능이 훨씬 낮음"
  ["None"]="없음"
  ["AI stays inert"]="AI는 동작하지 않음"
  ["NVIDIA GPU inference"]="NVIDIA GPU 추론"
  ["the CUDA runtime, ~4.7 GiB"]="CUDA 런타임, 약 4.7 GiB"
  ["Who is this machine for?"]="이 컴퓨터는 누구의 것입니까?"
  ["Username"]="사용자 이름"
  ["lower-case, no spaces"]="소문자, 공백 없이"
  ["Full name (optional)"]="전체 이름 (선택)"
  ["Password"]="비밀번호"
  ["Password again"]="비밀번호 다시"
  ["They do not match"]="일치하지 않습니다"
  ["the native compositor"]="자체 컴포지터"
  ["synui is not selected"]="synui가 선택되지 않음"
  ["headless"]="GUI 없음"
  ["Language, keyboard and time"]="언어, 키보드, 시간"
  ["Pick a language and the other three follow it. The console keymap and the desktop layout are separate on purpose — Swedish is 'sv-latin1' to the console and 'se' to the desktop — so they can be changed on their own afterwards."]="언어를 고르면 나머지 셋이 따라갑니다. 콘솔 키맵과 데스크톱 배치는 일부러 따로 둡니다 — 스웨덴어는 콘솔에서 'sv-latin1', 데스크톱에서 'se' 입니다 — 나중에 각각 따로 바꿀 수 있도록 하기 위해서입니다."
  ["Language"]="언어"
  ["sets the keyboard and the fonts too"]="키보드와 글꼴도 함께 설정합니다"
  ["typed by hand — fonts cover as much as possible"]="직접 입력 — 글꼴이 가능한 만큼 덮습니다"
  ["Sets the locale, both keyboard names and the font pack. Any locale glibc has can be typed instead."]="로캘, 두 가지 키보드 이름, 글꼴 묶음을 설정합니다. 대신 glibc가 아는 어떤 로캘이든 입력할 수 있습니다."
  ["The common zones first, then every name tzdata ships."]="흔히 쓰는 시간대가 먼저, 그다음 tzdata가 제공하는 모든 이름."
  ["Console keymap"]="콘솔 키맵"
  ["loadkeys — the text console and the greeter"]="loadkeys — 텍스트 콘솔과 로그인 화면"
  ["Every keymap this image can load. This one names a file loadkeys has to find, which is why it is not the same list as the desktop layout."]="이 이미지가 불러올 수 있는 모든 키맵. 여기서는 loadkeys가 찾아야 하는 파일 이름을 가리키므로, 데스크톱 배치와 같은 목록이 아닙니다."
  ["Desktop layout"]="데스크톱 배치"
  ["XKB — the compositor"]="XKB — 컴포지터"
  ["Desktop keyboard layout"]="데스크톱 키보드 배치"
  ["The layouts xkbcommon can compile. 'uk' is a console keymap and not a layout here — the layout is 'gb'."]="xkbcommon이 컴파일할 수 있는 배치. 'uk'는 콘솔 키맵이며 여기서는 배치가 아닙니다 — 배치 이름은 'gb'입니다."
  ["Read this back"]="다시 읽어 보기"
  ["Nothing has been written yet. The next button is the one that starts."]="아직 아무것도 기록되지 않았습니다. 다음 단추가 시작하는 단추입니다."
  ["EVERY PARTITION ON %1 WILL BE DELETED"]="%1의 모든 파티션이 삭제됩니다"
  ["SynapseOS will be installed into the free space on %1"]="SynapseOS는 %1의 빈 공간에 설치됩니다"
  ["SynapseOS is installed"]="SynapseOS가 설치되었습니다"
  ["The install stopped"]="설치가 중단되었습니다"
  ["Installing SynapseOS"]="SynapseOS 설치 중"
  ["Reboot and remove the installation media."]="다시 시작하고 설치 매체를 빼세요."
  ["The log below is the whole story — the last lines say why."]="아래 로그가 전부를 말해 줍니다 — 마지막 줄들이 이유를 알려 줍니다."
  ["This takes a while: the base system and the packages are downloaded, and an AI model is gigabytes on its own."]="시간이 걸립니다. 기본 시스템과 패키지를 내려받고, AI 모델만 해도 몇 기가바이트입니다."
  ["Back"]="뒤로"
  ["Next"]="다음"
  ["Reboot"]="다시 시작"
  ["Close"]="닫기"
  ["type to filter, or type a name that is not listed"]="입력하여 거르거나, 목록에 없는 이름을 입력하세요"
  ["Nothing to list on this image — type the name instead."]="이 이미지에는 나열할 것이 없습니다 — 대신 이름을 입력하세요."
  ["Nothing matches — the row below uses what you typed."]="일치하는 것이 없습니다 — 아래 줄은 입력한 내용을 씁니다."
  ["Use “%1” as typed"]="입력한 대로 “%1” 사용"
)
