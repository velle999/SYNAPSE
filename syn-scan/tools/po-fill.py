...
def main():
    ...
    for lineno, raw in enumerate(sys.stdin.read().split("\n"), 1):
        ...
        # ⛔ UNESCAPED THE SAME WAY THE .po SIDE IS, or a msgid containing a
        # QUOTE can never be matched. The .po's msgid goes through
        # po_unescape(), which turns \" into "; this side used to convert only
        # \n, so `\"Take ownership\" copies …` was refused as "no such msgid"
        # while sitting in the file three lines away. Nothing about the message
        # said the two sides were escaped differently.
        #
        # ⚠ THE msgstr SIDE STILL CANNOT USE po_unescape(): a plural entry is
        # tab-separated here, and \t is one of the sequences it would collapse.
        #
        # ⚠ THIS COPY THEREFORE DIFFERS from the ones in syn-edit/, synfiles/,
        # synpkg/, syn-cal/, syn-settings/ and synstudio/. Their catalogs are
        # full, so the bug cannot reach them retroactively; it bites the next
        # person who marks a string with a quote in it.
        mid = po_unescape(mid)
        if mid not in index:
