#!/usr/bin/env bash
# pot.sh — syn-scan's message template, from BOTH of its source languages.
#
...
#
# ⛔ THE RECORD PROTOCOL IS NOT IN HERE. `--rec` emits header rows naming the
# columns — `engine verdict path detail when`, `id name present runnable path`,
# `id origin engine detail` — and data/syn-scan.qml keys off those names.
#
# ⚠ TWO COLUMNS ARE MATCHED RATHER THAN READ, and they are the trap here.
# `engine` is what `syn-scan scan --only clamav` takes back, and `verdict` is
# what the window compares against "infected" to colour a row red. A translated
# verdict is a window drawing an infected file as clean. Every engine carries
# an id AND a name one struct field apart — `clamav` and "ClamAV" — and only
# the second is marked.
#
# ⛔ AND A SIGNATURE NAME IS NEVER MARKED. `Win.Test.EICAR_HDB-1` is upstream
# ClamAV's string; it is quoted, not translated.
...
msgcat --use-first --no-location -o "$out/syn-scan.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/syn-scan.pot")" \
