#!/usr/bin/env bash
# pot.sh — syn-edit's message template, from BOTH of its source languages.
#
...
#
# ⛔ THE RECORD PROTOCOL IS NOT IN HERE. `--rec` emits keys the window matches
# on and values like ed_mode_name()'s "INSERT"; none of them is marked, and
# tests/i18n_test.sh proves it by running the record commands under a real
...
msgcat --use-first --no-location -o "$out/syn-edit.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/syn-edit.pot")" \
