#!/usr/bin/env bash
# pot.sh — synpkg's message template, from BOTH of its source languages.
#
...
#
# ⛔ THE --tsv PATH IS NOT IN HERE. Every _() in the C sits on the human side of
# a `g_out == OUT_TSV` branch; tests/i18n_test.sh proves it by running the TSV
# commands under a real foreign locale and diffing.
#
# SynapseOS Project
...
msgcat --use-first --no-location -o "$out/synpkg.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/synpkg.pot")" \
