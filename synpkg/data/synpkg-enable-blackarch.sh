#!/usr/bin/env bash
#
# synpkg-enable-blackarch — add the BlackArch repository to a running SynapseOS.
#
# Invoked by `synpkg arsenal enable-repo`. Kept as shell rather than folded
# into the C binary because every line of it is orchestration of curl, tar,
# gpg and pacman, and because the mirror-redundancy logic below is the fix for
# a real outage (2026-08-07). Rewriting that in C would risk it for nothing.
#
...
[ "$(id -u)" -eq 0 ] || die "must run as root (try: sudo synpkg arsenal enable-repo)"
...
msg "Browse them with: synpkg arsenal   (or the SYNAPSE Software app)"
