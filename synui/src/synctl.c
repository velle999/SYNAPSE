...
int main(int argc, char **argv)
{
    if (argc < 2) {
        fprintf(stderr,
            ...
            "  cursor                 the pointer's position, {\"x\":,\"y\":}\n"
            "  pointer                who is holding the mouse: every pointer\n"
            "                         constraint on file, which one is active\n"
            ...
            "                         module and the desktop widget all share.\n"
            "                         on/off is the NETWORK switch, not furniture\n"
            "  hdr                    whether each monitor can be driven in\n"
            "                         HDR10, asked of the hardware without\n"
            "                         changing anything on screen\n"
            "  hdr on|off <output>    drive that monitor in HDR10, or stop.\n"
            "                         Answers whether it WORKED — a link or a\n"
            "                         mode that will not carry PQ is an error,\n"
            "                         not a silent no-op\n"
            "  hdr white <output> <n> where SDR white sits on PQ's absolute\n"
            "                         scale, in cd/m2 (80-600, default 203).\n"
            "                         Not a brightness control: it moves only\n"
            "                         where SDR content lands inside HDR\n"
            "  virtual                every virtual display, and which\n"
            "                         output (if any) is solo\n"
            "  virtual add <WxH[@Hz]> [scale]\n"
            "                         a display with no monitor behind it —\n"
            "                         what `syn-remote stream` serves\n"
            "  virtual mode <name> <WxH[@Hz]>\n"
            "                         resize one — how a stream follows the\n"
            "                         resolution its client asked for\n"
            "  virtual remove <name>|all   take one away\n"
            "  virtual solo <name>|off     keep that one screen lit and turn\n"
            "                         every other one off\n"
