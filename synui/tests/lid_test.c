 ...
 *   - which of the two settings a closed lid uses (docked or not)
...
int main(void)
{
    ...
    /* 1. Undocked + suspend: the systemd default, done by synui. */
    reset(false);
    ...
    /* 5. system: synui holds no inhibitor and must do nothing at all — logind
     *    has already acted by the time we hear about it. */
    reset(false);
    ...
    /* 7. Repeats are ignored — a switch that re-reports its state must not
     *    suspend twice. */
    reset(false);
    srv->config.lid_close_action = SYN_LID_SUSPEND;
    power_lid_set(srv, true);
    ...
    /* 8. power.state round-trips names, not indices. */
    reset(false);
    ...
    /* 9. An action name power.state does not know leaves the configured value
    ...
    /* 10. Every action name parses back to its own enum value — the table and
     *     the enum drifting apart is the one way all of the above still
     *     passes while the config file means something else. */
