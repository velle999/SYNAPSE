...
# toolkits synui does not draw: GTK apps, Dolphin (Qt/KDE), the terminals
# (syntty and kitty) and Firefox.
#
...
# ── syntty (the default terminal) ────────────────────────────────────────────
#
# ⚠ THE ONE THE DESKTOP OPENS BY ITSELF, and until now the one window on the
# screen that ignored the theme entirely. On a Win95 desktop — silver windows,
# navy captions, black text everywhere — syntty came up #1B1F26 dark grey with
# pale text, because it read nothing from here and fell back to its built-in
# scheme. Every other surface had switched.
#
# Same split as kitty and for the same reason: the palette is regenerated on
# every theme switch, so it goes in its OWN file and `syntty.conf` — which the
# user owns, and which synui-apply-font also edits — only gains one line.
#
# ⚠ IT IS `include = path`, WITH THE EQUALS. syntty's config format is
# `key = value` with no exceptions, deliberately; kitty's bare `include path`
# is a syntax error there (a helpful one — it names the difference — but an
# error, and it would leave the whole palette unread).
#
# ⚠ APPENDED, NOT PREPENDED. syntty takes the last assignment for a key exactly
# as kitty does, so an include at the top would lose to any colour the user has
# in their own file and the theme would appear to do nothing.
#
# The colours themselves are DERIVED FROM $ansi ABOVE rather than written out
# again. Two hand-typed copies of sixteen colours are two copies that drift,
# and the direction they drift in is one terminal quietly disagreeing with the
# other about what "red" is.
#
# What is NOT written here, and why:
#   - a SELECTION colour. syntty's selection is an INVERSION of whatever is
#     underneath it, by construction, so it contrasts with any program's own
#     colours. Handing it a fixed pair would be a downgrade.
#   - TAB BAR colours. Its tab bar draws in the terminal's own foreground and
#     background (render_colors_get), so it follows the two keys below already.
#   - OPACITY. syntty has no alpha key: it is not in synui's glass list and
#     takes the compositor's uniform fade like any other opaque window.
#     synui-glass owns that question for the terminals that do.
if command -v syntty >/dev/null 2>&1; then
    sdir="$HOME/.config/syntty"
    scol="$sdir/synui-colors.conf"
    sconf="$sdir/syntty.conf"
    if mkdir -p "$sdir" 2>/dev/null; then
        IFS=, read -r sbr sbg sbb <<<"$bar_base"
        s_bg=$(rgb_hex "$sbr" "$sbg" "$sbb")
        s_accent=$(rgb_hex "$ar" "$ag" "$ab")
        IFS=, read -r sir sig sib <<<"$(ink_for "$ar" "$ag" "$ab")"
        s_on_accent=$(rgb_hex "$sir" "$sig" "$sib")

        {
            echo "# Generated by synui-apply-theme — do not edit; a theme switch overwrites it."
            echo "# Included from syntty.conf. syntty re-reads this file when it changes,"
            echo "# so an already-open terminal recolours in place."
            echo "background  = $s_bg"
            echo "foreground  = $fg"
            echo "cursor      = $s_accent"
            echo "cursor_text = $s_on_accent"
            echo ""
            # `color0  #21222c` -> `color0 = #21222c`. One source, two syntaxes.
            printf '%s\n' "$ansi" | sed -E 's/^(color[0-9]+)[[:space:]]+(#.*)$/\1 = \2/'
        } > "$scol.tmp"
        mv -f "$scol.tmp" "$scol" 2>/dev/null || rm -f "$scol.tmp"

        # syntty ships no default config — it runs on built-in defaults — so the
        # file has to exist for the include to be read at all. Created with a
        # comment rather than empty so somebody opening it can tell what put it
        # there; `syntty config --example` is what documents the rest.
        [ -e "$sconf" ] || printf '# Created by synui-apply-theme.\n' > "$sconf"
        if [ -f "$sconf" ] && ! grep -q '^[[:space:]]*include[[:space:]]*=[[:space:]]*synui-colors\.conf' "$sconf"; then
            printf '\n# Desktop theme colours, regenerated by synui-apply-theme.\ninclude = synui-colors.conf\n' >> "$sconf"
        fi

        # ⚠ NO SIGNAL IS SENT, and that is deliberate rather than an omission.
        # kitty and foot are told to reload with SIGUSR1; syntty WATCHES the
        # file instead, so the write above is the whole notification. Sending
        # SIGUSR1 anyway would be actively harmful: a syntty predating the
        # watch has no handler for it, and the default disposition of SIGUSR1
        # is TERMINATE — the first theme switch after an update would kill
        # every terminal that was already open.
        :
    fi
fi
...
#
# Same split as kitty, for the same reason: the palette goes in its OWN file and
# `config.rasi` — which the user is expected to own — only gains an @import line
# if it does not already have one. Appended rather than prepended because rofi
# takes the LAST definition of a property, so a theme that lost to a stale line
# the user forgot they wrote would be a theme switch that silently does nothing.
#
# Colours are written as #rrggbb rather than rasi's rgba() because the popup
# alpha belongs to the WINDOW, not to every element that inherits from it —
# putting alpha on the shared `*` block makes the text translucent too, which on
# a 0.97 surface reads as a rendering bug rather than as glass.
if command -v rofi >/dev/null 2>&1; then
    ...
    if mkdir -p "$rdir" 2>/dev/null; then
        ...
        r_accent=$(rgb_hex "$ar" "$ag" "$ab")
        IFS=, read -r rir rig rib <<<"$(ink_for "$ar" "$ag" "$ab")"
        r_on_accent=$(rgb_hex "$rir" "$rig" "$rib")

        # The row you are on has to be legible, and "accent background + a
        # foreground picked for contrast against it" is the only pairing that
        # holds for every theme in the set — SYNAPSE's neon magenta, 95's
        # silver and XP's beige do not agree on whether white or black reads.
        # ink_for() answers that per theme instead of guessing once here.
        cat > "$rtheme" <<RASI
/* Generated by synui-apply-theme — do not edit; a theme switch overwrites it. */
* {
...
RASI
        ...
        # carries no $HOME and the file stays valid if the directory is copied
        # to another machine or another user.
        if [ ! -e "$rconf" ]; then
            printf '@import "synui.rasi"\n' > "$rconf"
        elif ! grep -q 'synui\.rasi' "$rconf" 2>/dev/null; then
            printf '\n@import "synui.rasi"\n' >> "$rconf"
        fi
    fi
fi
