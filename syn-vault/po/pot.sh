#!/usr/bin/env bash
# pot.sh — syn-vault's message template, from BOTH of its source languages.
#
...
#
# ⛔ THE RECORD PROTOCOL IS NOT IN HERE. `--rec` emits a header row NAMING THE
# COLUMNS — `name`, `state`, `mount`, `stray` — and data/syn-vault.qml keys off
# those names and matches `state === "open"` in seven places to decide whether a
# row is a vault you can lock or one you can unlock. None of them is marked, and
# tests/i18n_test.sh proves it by running every offline --rec command under a
# catalog that translates EVERYTHING and diffing the bytes.
#
# ⚠ AND `open`/`locked` ARE ALSO DRAWN, which is the trap this component had:
# the same two words are the record's identifiers AND what the CLI prints in a
# sentence. Two spellings, and only the drawn one is marked.
...
msgcat --use-first --no-location -o "$out/syn-vault.pot" "$tmp/qml.pot" "$tmp/c.pot"
printf 'pot.sh: %s msgids (%s from QML, %s from the C)\n' \
    "$(grep -c '^msgid ' "$out/syn-vault.pot")" \
