...
| **`syn-vault`** | **The file vault** — a password-locked folder for your own files. A vault's contents live encrypted in `~/.local/share/syn-vault/<name>.vault` and are readable only while it is open, at `~/Vaults/<name>`. **It contains no cryptography and must not:** gocryptfs does the encrypting, chosen over a LUKS container because it is genuinely *userspace* — no root, no polkit prompt on every unlock, just FUSE and your own permissions. **The password never touches a command line** (`/proc/<pid>/cmdline` is world-readable), so there is no `--password` option: it is read from the terminal with echo off, or from stdin, which is how the window passes one in. Opening a vault over a mountpoint that already has files in it is **refused** — mounting hides files rather than encrypting them, and that is how somebody comes to believe an unencrypted file is protected; `list` and `status` flag the same condition. `syn-vault gui`, a **Vault** entry in synfiles' sidebar, and an applications-menu entry of its own. |
...
| **`synapse-voice`** | The speech engine the desktop shares: dictation (`Super`+`Shift`+`V`), the assistant's and the screen reader's piper voice, and chibi's ears. faster-whisper and piper with their models, so it works offline (~900 MB). Without it the screen reader falls back to espeak-ng and dictation says what is missing. |
...
| `Super`+`W` / `Super`+`Shift`+`W` | Wallpaper picker (`Tab` scopes it to one monitor, `w` opens the wallhaven browser) / reload the wallpaper |
| `Super`+`Ctrl`+`W` | Browse wallhaven.cc for a wallpaper (`w` goes back to the picker) |
...
The **Malware Scanning** pane is the switch for the weekly sweep, and the
answer to "has this machine been scanned". It owns none of the scanning —
`syn-scan` does, and the pane reads its records — but it is where the
schedule, the signature updates and ClamAV's resident daemon are turned on and
off, and where the three engines say whether they are installed, ready, or
present but runnable only as root — which is how Arch ships `rkhunter`.

⚠ The scheduled sweep and a scan you type are two records in two places: the
...
random, paged. Picking one downloads it into `~/Pictures/Wallpapers` — the first
directory the picker scans — and sets it, so a wallpaper taken from wallhaven is
an ordinary local wallpaper from then on.

**`w` flips between the two.** The picker's header carries a **[w] Wallhaven**
...
> the internet at all. The window opens on that question the first time and
> browses nothing until you answer it — `synui-wallhaven on` is the same switch
> from a prompt. Results are pinned to wallhaven's `sfw` purity, which is not a
> setting: the other levels need an API key, and a wallpaper picker that asks
> you to paste a credential into it is a promise this would rather not make.

`synui-wallhaven` is the command-line half — `search`, `get`, `set`, and the
switch:

```bash
synui-wallhaven on                       # the opt-in (or the window's own button)
